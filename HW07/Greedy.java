...
public interface Greedy<T>{
    ...
    default List<T> greedyAlgorithm(){
        ...
        Iterator<T> element_iterator = selection();
        T element;
        while (element_iterator.hasNext()){
            element = element_iterator.next();
            if (feasibility(candidates_list, element)) assign(candidates_list, element);
            if (solution(candidates_list)) return candidates_list;
        }
